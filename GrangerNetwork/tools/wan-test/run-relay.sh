...
if [[ -z "$STATE_DIR" || -z "$LISTEN_HOST" || -z "$LISTEN_PORT" || ${#CAPABILITIES[@]} -eq 0 ]]; then
  echo "Usage: run-relay.sh --state-dir DIR --listen-host NUMERIC_IP --listen-port PORT --capability ROLE [--capability ROLE]" >&2
  exit 2
fi
export PYTHONPATH="$NETWORK_ROOT/src${PYTHONPATH:+:$PYTHONPATH}"
if [[ ! -f "$STATE_DIR/node-descriptor.json" ]]; then
  ARGS=(-m granger_network.node init --state-dir "$STATE_DIR" \
    --listen-host "$LISTEN_HOST" --listen-port "$LISTEN_PORT" \
    --descriptor-lifetime "$DESCRIPTOR_LIFETIME" \
    --max-connections "$MAX_CONNECTIONS" --max-circuits "$MAX_CIRCUITS")
  for role in "${CAPABILITIES[@]}"; do ARGS+=(--capability "$role"); done
  "$PYTHON_BIN" "${ARGS[@]}"
fi
